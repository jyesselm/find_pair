...
using config::params::EMPTY_CRITERION;
using config::params::MAX_VALID_SCORE;
...
void JsonWriter::record_hbond_list(size_t base_i, size_t base_j, const std::vector<core::hydrogen_bond>& hbonds) {
    // NOTE: We receive 0-based indices, but need to output 1-based for legacy compatibility
    // Legacy outputs BOTH (i,j) and (j,i) for each pair, like record_distance_checks does.
    // For the reversed pair, donor/acceptor atoms are swapped.

    // First record: (base_i, base_j)
    nlohmann::json record;
    ...
    // Second record: (base_j, base_i) with swapped donor/acceptor
    // Legacy also outputs the reversed pair
    nlohmann::json record_reversed;
    record_reversed["type"] = "hbond_list";
    record_reversed["base_i"] = static_cast<long>(base_j + 1); // Swap: j becomes i
    record_reversed["base_j"] = static_cast<long>(base_i + 1); // Swap: i becomes j
    record_reversed["num_hbonds"] = hbonds.size();

    nlohmann::json hbonds_reversed_array = nlohmann::json::array();
    for (size_t i = 0; i < hbonds.size(); ++i) {
        const auto& hbond = hbonds[i];
        nlohmann::json hbond_json;
        // Swap donor and acceptor for reversed pair
        hbond_json["donor_atom"] = hbond.acceptor_atom;
        hbond_json["acceptor_atom"] = hbond.donor_atom;
        hbond_json["distance"] = format_double(hbond.distance);
        hbond_json["type"] = std::string(1, hbond.type);
        if (hbond.hbond_idx.has_value()) {
            hbond_json["hbond_idx"] = static_cast<long>(hbond.hbond_idx.value());
        } else {
            hbond_json["hbond_idx"] = static_cast<long>(i + 1);
        }
        hbonds_reversed_array.push_back(hbond_json);
    }
    record_reversed["hbonds"] = hbonds_reversed_array;
