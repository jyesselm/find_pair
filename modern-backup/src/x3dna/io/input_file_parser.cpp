...
InputData InputFileParser::parse_stream(std::istream& stream) {
    ...
    while (std::getline(stream, line)) {
        ...
        try {
            ...
            // Create base pair with identity frames (will be recalculated during analysis)
            // Input file parsing doesn't have frame data - frames come from residues
            core::ReferenceFrame identity_frame;
            core::BasePair bp(res1, res2, identity_frame, identity_frame, core::BasePairType::UNKNOWN);
