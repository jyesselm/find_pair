...
ValidationResult BasePairValidator::validate(const Residue& res1, const Residue& res2) const {
    ...
    if (cdns) {
        ...
        // Using HBondDetector::count_potential_hbonds (verified to match legacy exactly)
        hbond_detector_.count_potential_hbonds(res1, res2, result.num_base_hb, result.num_o2_hb);
        ...
        // Find detailed H-bonds if we have any H-bonds (BEFORE is_valid check)
        // Legacy records H-bonds for ALL pairs that pass cdns + overlap + hbond_check
        // This is done via adjust_pairQuality which is called BEFORE bpid check
...
std::vector<core::hydrogen_bond> BasePairValidator::find_hydrogen_bonds(const Residue& res1,
    ...
    // Use HBondDetector to get ALL H-bonds including invalid ones (type=' ')
    // This matches legacy behavior where ALL H-bonds are recorded to JSON

    HBondPipelineResult detailed = hbond_detector_.detect_all_hbonds_detailed(res1, res2);

    // Convert from core::HBond to core::hydrogen_bond format
    // Return all_classified_bonds which includes ALL H-bonds (including INVALID)
    std::vector<core::hydrogen_bond> all_hbonds;
    for (const auto& hb : detailed.all_classified_bonds) {
        core::hydrogen_bond hbond;
        hbond.donor_atom = hb.donor_atom_name;
        hbond.acceptor_atom = hb.acceptor_atom_name;
        // Use absolute distance (matches legacy fabs(hb_dist[i]) in json_writer.c:1165)
        hbond.distance = std::abs(hb.distance);
        // Convert classification to type char
        if (hb.classification == core::HBondClassification::STANDARD) {
            hbond.type = '-';
        } else if (hb.classification == core::HBondClassification::NON_STANDARD) {
            hbond.type = '*';
        } else {
            hbond.type = ' ';
        }
...
char BasePairValidator::donor_acceptor(char base1, char base2, const std::string& atom1, const std::string& atom2) {
    ...
    // BASE_LIST = "ACGITU" (A=0, C=1, G=2, I=3, T=4, U=5)
    static const char* BASE_LIST = "ACGITU";
    ...
    // Find base indices in BASE_LIST
    int inum = -1, jnum = -1;
    const char* pchar = strchr(BASE_LIST, std::toupper(static_cast<unsigned char>(base1)));
    if (pchar)
        inum = pchar - BASE_LIST;
    pchar = strchr(BASE_LIST, std::toupper(static_cast<unsigned char>(base2)));
    if (pchar)
        jnum = pchar - BASE_LIST;
...
bool BasePairValidator::is_base_atom(const std::string& atom_name) {
    return core::atom_classification::is_base_atom_for_hbond(atom_name);
}
