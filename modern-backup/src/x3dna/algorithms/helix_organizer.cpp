...
[[nodiscard]] double dot2ang(double d) {
 ...
 */
[[nodiscard]] FrameAlignment compute_frame_alignment(const core::ReferenceFrame& frame1,
 ...
 */
[[nodiscard]] bool are_on_opposite_z_sides(double d1, double d2) {
    return d1 * d2 < 0;
}

/**
 * @brief Context for processing a single helix during five2three algorithm
 *
 * Groups parameters to reduce function signature complexity.
 */
struct HelixProcessingContext {
...
/**
 * @brief Result of checking whether a step needs a swap
 */
struct StepSwapResult {
    bool needs_swap = false;
    bool from_wc = false;
    bool from_o3d = false;
    bool from_schain = false;
    bool from_others = false;
    bool from_chain1dir = false;
};
...
bool has_positive_bpid(const x3dna::core::BasePair& pair) {
    ...
    // Get frames directly (frames are always present in a valid BasePair)
...
LinkDirection HelixOrganizer::check_linkage(size_t res_i, size_t res_j, const BackboneData& backbone) const {
...
bool HelixOrganizer::are_pairs_backbone_connected(const core::BasePair& pair1, const core::BasePair& pair2,
...
double HelixOrganizer::wcbp_zdir(const core::BasePair& pair_m, const core::BasePair& pair_n, bool swap_m,
                                 bool swap_n) const {
...
void HelixOrganizer::first_step(const std::vector<core::BasePair>& pairs, const BackboneData& backbone,
...
bool HelixOrganizer::wc_bporien(const core::BasePair& pair_m, const core::BasePair& pair_n, bool swap_m, bool swap_n,
                                const BackboneData& backbone) const {
...
bool HelixOrganizer::check_o3dist(const core::BasePair& pair_m, const core::BasePair& pair_n, bool swap_m, bool swap_n,
                                  const BackboneData& backbone) const {
    auto res_m = get_strand_residues(pair_m, swap_m);
    auto res_n = get_strand_residues(pair_n, swap_n);
...
bool HelixOrganizer::check_schain(const core::BasePair& pair_m, const core::BasePair& pair_n, bool swap_m, bool swap_n,
                                  const BackboneData& backbone) const {
    auto res_m = get_strand_residues(pair_m, swap_m);
    auto res_n = get_strand_residues(pair_n, swap_n);
...
bool HelixOrganizer::check_others(const core::BasePair& pair_m, const core::BasePair& pair_n, bool swap_m, bool swap_n,
                                  const BackboneData& backbone) const {
    auto res_m = get_strand_residues(pair_m, swap_m);
    auto res_n = get_strand_residues(pair_n, swap_n);

    // If any backbone linkage exists, no swap needed
    const bool has_any_linkage = check_linkage(res_m.strand1, res_n.strand1, backbone) != LinkDirection::None ||
                                 check_linkage(res_m.strand2, res_n.strand2, backbone) != LinkDirection::None ||
...
bool HelixOrganizer::chain1dir(const core::BasePair& pair_m, const core::BasePair& pair_n, bool swap_m, bool swap_n,
                               const BackboneData& backbone) const {
    auto res_m = get_strand_residues(pair_m, swap_m);
    auto res_n = get_strand_residues(pair_n, swap_n);
...
DirectionCounts HelixOrganizer::check_direction(const std::vector<core::BasePair>& pairs, const BackboneData& backbone,
...
// =============================================================================
// check_strand2 helper functions
// =============================================================================

void HelixOrganizer::check_strand2_normal(const std::vector<core::BasePair>& pairs, const BackboneData& backbone,
                                          const std::vector<size_t>& pair_order, const HelixSegment& helix,
                                          std::vector<bool>& swapped) const {
    for (size_t pos = helix.start_idx; pos < helix.end_idx; ++pos) {
        size_t idx_m = pair_order[pos];
        size_t idx_n = pair_order[pos + 1];
...
void HelixOrganizer::check_strand2_mixed(const std::vector<core::BasePair>& pairs, const BackboneData& backbone,
    ...
    for (size_t pos = helix.start_idx; pos < helix.end_idx; ++pos) {
        ...
        const bool no_same_strand_links = check_linkage(res_m.strand1, res_n.strand1, backbone) ==
                                              LinkDirection::None &&
                                          check_linkage(res_m.strand2, res_n.strand2, backbone) == LinkDirection::None;
        ...
        const bool should_swap_m = (anti_p &&
                                    check_linkage(res_m.strand2, res_n.strand1, backbone) == LinkDirection::Forward) ||
                                   (parallel &&
                                    check_linkage(res_m.strand1, res_n.strand2, backbone) == LinkDirection::Reverse);

        const bool should_swap_n = (anti_p &&
                                    check_linkage(res_m.strand1, res_n.strand2, backbone) == LinkDirection::Forward) ||
                                   (parallel &&
                                    check_linkage(res_m.strand2, res_n.strand1, backbone) == LinkDirection::Reverse);
...
// =============================================================================
// Main check_strand2 function
// =============================================================================

void HelixOrganizer::check_strand2(const std::vector<core::BasePair>& pairs, const BackboneData& backbone,
                                   const std::vector<size_t>& pair_order, HelixSegment& helix,
                                   std::vector<bool>& swapped, const DirectionCounts& direction) const {
    ...
    if (!mixed_direction) {
        // Check if there are any linkages to process
        if (direction.strand1_forward + direction.strand1_reverse + direction.strand2_forward +
                direction.strand2_reverse ==
            0) {
            return;
        }
        check_strand2_normal(pairs, backbone, pair_order, helix, swapped);
    } else {
        check_strand2_mixed(pairs, backbone, pair_order, helix, swapped, direction);
    }
}
...
// =============================================================================
// calculate_context helper functions
// =============================================================================

std::vector<std::pair<double, size_t>> HelixOrganizer::find_nearby_pairs(
    size_t pair_idx, const std::vector<core::BasePair>& pairs) const {
    std::vector<std::pair<double, size_t>> neighbors;
    auto org_i = get_pair_origin(pairs[pair_idx]);
...
std::optional<size_t> HelixOrganizer::try_find_indirect_neighbor2(
    const std::vector<std::pair<double, size_t>>& neighbors, const std::vector<core::BasePair>& pairs,
    const geometry::Vector3D& z_i, double d1) const {
...
// =============================================================================
// Main calculate_context function
// =============================================================================

std::vector<HelixOrganizer::PairContext> HelixOrganizer::calculate_context(const std::vector<core::BasePair>& pairs,
                                                                           const BackboneData& backbone) const {
    ...
    for (size_t i = 0; i < n; ++i) {
        ...
        std::vector<std::pair<double, size_t>> neighbors = find_nearby_pairs(i, pairs);
        ...
        context[i].has_backbone_link1 = are_pairs_backbone_connected(pairs[i], pairs[neighbors[0].second], backbone);
        ...
        for (size_t k = 1; k < neighbors.size(); ++k) {
            ...
            if (are_on_opposite_z_sides(d1, dk)) {
                ...
                context[i].has_backbone_link2 = are_pairs_backbone_connected(pairs[i], pairs[neighbors[k].second],
                                                                             backbone);
        ...
        if (!context[i].neighbor2.has_value()) {
            ...
            // Legacy special case: try to find neighbor2 indirectly through neighbor1
            auto indirect_n2 = try_find_indirect_neighbor2(neighbors, pairs, z_i, d1);
            if (indirect_n2.has_value()) {
                context[i].neighbor2 = indirect_n2.value();
                context[i].dist2 = neighbors[1].first;
                context[i].has_backbone_link2 = are_pairs_backbone_connected(pairs[i], pairs[indirect_n2.value()],
                                                                             backbone);
            }
        }
    }
...
std::vector<size_t> HelixOrganizer::find_endpoints(const std::vector<PairContext>& context) const {
...
// =============================================================================
// locate_helices helper functions
// =============================================================================

bool HelixOrganizer::should_skip_endpoint(size_t ep, const PairContext& ep_ctx,
    ...
    return matched_count == item_count;
}

void HelixOrganizer::add_endpoint_and_neighbors(size_t ep, const PairContext& ep_ctx, std::vector<size_t>& pair_order,
...
void HelixOrganizer::traverse_helix(const std::vector<PairContext>& context, std::vector<size_t>& pair_order,
                                    std::vector<bool>& visited, size_t helix_start) const {
    // Continue adding neighbors until we can't anymore
    while (pair_order.size() >= helix_start + 1) {
        size_t ip = pair_order.size() - 1;
        size_t current = pair_order[ip];
        const auto& ctx = context[current];

        // If this is an endpoint, only add neighbor1 if conditions met, then stop
        if (ctx.is_endpoint) {
        ...
        // Not an endpoint - check if we should continue
        ...
        // Stop if both matched or neither matched (when both neighbors exist)
        ...
        // Go to the other neighbor (opposite of where we came from)
...
std::vector<size_t> HelixOrganizer::collect_leftover_pairs(const std::vector<bool>& visited, size_t num_pairs) const {
...
std::pair<std::vector<size_t>, std::vector<HelixSegment>> HelixOrganizer::locate_helices(
    ...
    // Process each endpoint to build helices
    for (size_t ep : endpoints) {
        if (debug) {
        ...
        if (should_skip_endpoint(ep, ep_ctx, visited)) {
            continue;
        }
        ...
        // Add endpoint and its neighbors
        add_endpoint_and_neighbors(ep, ep_ctx, pair_order, visited);

        // Traverse helix from current position
        traverse_helix(context, pair_order, visited, helix.start_idx);
    ...
    std::vector<size_t> leftover = collect_leftover_pairs(visited, num_pairs);
    if (!leftover.empty()) {
        HelixSegment helix;
        helix.start_idx = pair_order.size();
...
// =============================================================================
// ensure_five_to_three helper functions
// =============================================================================

void HelixOrganizer::first_pass_strand_check(const std::vector<core::BasePair>& pairs, const BackboneData& backbone,
                                             const std::vector<size_t>& pair_order, const HelixSegment& helix,
                                             std::vector<bool>& swapped) const {
    bool debug = is_five2three_debug_enabled();
...
void HelixOrganizer::second_pass_wc_check(const std::vector<core::BasePair>& pairs, const BackboneData& backbone,
                                          const std::vector<size_t>& pair_order, const HelixSegment& helix,
                                          std::vector<bool>& swapped) const {
    bool debug = is_five2three_debug_enabled();
...
void HelixOrganizer::ensure_five_to_three(const std::vector<core::BasePair>& pairs, const BackboneData& backbone,
    ...
    for (auto& helix : helices) {
        ...
        // STEP 2: First pass - check orientation and distance for consecutive pairs
        first_pass_strand_check(pairs, backbone, pair_order, helix, swapped);

        // STEP 3: Second pass - re-check WC orientation
        second_pass_wc_check(pairs, backbone, pair_order, helix, swapped);
...
HelixOrdering HelixOrganizer::organize(const std::vector<core::BasePair>& pairs, const BackboneData& backbone) const {
    ...
    // All pairs have frames (required by BasePair constructor)
