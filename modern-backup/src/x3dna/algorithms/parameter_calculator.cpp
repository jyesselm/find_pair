...
core::HelicalParameters ParameterCalculator::calculate_helical_parameters_impl(
    ...
    // Calculate tip_inc1: angle between axis_h and rot1 z-axis
    double tip_inc1 = magang(axis_h, z1);
    ...
    geometry::Matrix3D temp_rot = arb_rotation(hinge1, -tip_inc1);
    geometry::Matrix3D rot1_h = temp_rot * rot1;

    // Calculate tip_inc2: angle between axis_h and rot2 z-axis
    double tip_inc2 = magang(axis_h, z2);
    ...
    temp_rot = arb_rotation(hinge2, -tip_inc2);
    ...
    geometry::Matrix3D mst_orien_h = x_y_z_2_mtx(h_x, h_y, axis_h);
    ...
    params.tip = tip_inc1 * std::cos(phi);
    params.inclination = tip_inc1 * std::sin(phi);
    ...
    geometry::Vector3D mst_org_h = (org1_h + org2_h) * 0.5;
    params.midstep_frame = core::ReferenceFrame(mst_orien_h, mst_org_h);
