...
std::vector<BasePair> BasePairFinder::find_all_pairs(const Structure& structure) const {
    ...
    for (size_t i = 0; i < nucleotide_residues.size(); ++i) {
        for (size_t j = i + 1; j < nucleotide_residues.size(); ++j) {
            ...
            if (result.is_valid) {
                // Skip pairs where either residue lacks a frame (non-nucleotides)
                if (!res1->reference_frame().has_value() || !res2->reference_frame().has_value()) {
                    continue;
                }

                BasePair pair(idx1, idx2, res1->reference_frame().value(), res2->reference_frame().value(),
                              result.bp_type);
...
std::optional<std::pair<int, ValidationResult>> BasePairFinder::find_best_partner(
    ...
    for (int idx2 = 1; idx2 <= ctx.mapping.max_legacy_idx; ++idx2) {
        ...
        // Record H-bonds for ALL pairs that have them (before is_valid check)
        // Legacy records H-bonds for every pair checked in best_pair, even if not valid
        // This matches legacy behavior where check_pair is called for all pairs in best_pair loop
        if (ctx.writer && legacy_idx1 < idx2 && !result.hbonds.empty()) {
            size_t base_i = static_cast<size_t>(legacy_idx1 - 1);
            size_t base_j = static_cast<size_t>(idx2 - 1);
            ctx.writer->record_hbond_list(base_i, base_j, result.hbonds);
        }
        ...
        // Only record when i < j to avoid duplicates - json_writer outputs both directions
...
void BasePairFinder::record_validation_results(int legacy_idx1, int legacy_idx2, const core::Residue* res1,
    ...
    if (passes_cdns) {
        ...
        if (result.is_valid && legacy_idx1 < legacy_idx2) {
            ...
            // Only record pairs where both residues have frames (nucleotides)
            if (res1->reference_frame().has_value() && res2->reference_frame().has_value()) {
                BasePair pair(base_i, base_j, res1->reference_frame().value(), res2->reference_frame().value(),
                              result.bp_type);
                pair.set_hydrogen_bonds(result.hbonds);
    ...
    // Note: H-bond recording is now done in find_best_partner before is_valid check
    // to match legacy behavior where check_pair is called for all pairs in best_pair loop
}
...
std::optional<double> check_nt_type_by_rmsd(const Residue& residue) {
    ...
    int nitrogen_count = 0; // Count of nitrogen atoms (N1, N3, N7, N9)
    ...
    // Legacy requires: (!nitrogen_count && !C1_prime) -> return DUMMY
    if (nitrogen_count == 0 && !has_c1_prime) {
...
BasePair BasePairFinder::create_base_pair(int legacy_idx1, int legacy_idx2, const Residue* res1, const Residue* res2,
    ...
    // Get frames from residues (caller must ensure both have frames)
    // Store by value since reference_frame().value() returns a temporary
    auto frame_small = res_small->reference_frame().value();
    auto frame_large = res_large->reference_frame().value();

    BasePair pair(idx_small, idx_large, frame_small, frame_large, result.bp_type);
