/**
 * @file detector.cpp
 * @brief Implementation of stateful H-bond detector
 */
...
/**
 * @brief Helper to check if residue has a specific atom
 */
bool has_atom(const Residue& residue, const char* name) {
...
HBondPipelineResult HBondDetector::detect_base_hbonds_detailed(const Residue& residue1, const Residue& residue2) const {
    HBondPipelineResult result;

    // Step 1: Find candidate bonds (base atoms only)
    result.initial_candidates = find_candidate_bonds(residue1, residue2, true);
    ...
    // Step 3: Get base types for classification
    const char base1 = get_base_type_for_hbond(residue1);
    const char base2 = get_base_type_for_hbond(residue2);
...
std::vector<HBond> HBondDetector::detect_all_hbonds_between(const Residue& residue1, const Residue& residue2) const {
    auto result = detect_all_hbonds_detailed(residue1, residue2);
    return result.final_bonds;
}

HBondPipelineResult HBondDetector::detect_all_hbonds_detailed(const Residue& residue1, const Residue& residue2) const {
    HBondPipelineResult result;

    // Step 1: Find candidate bonds (all atoms including backbone/sugar)
    result.initial_candidates = find_candidate_bonds(residue1, residue2, false);
    ...
    // Step 7: Store ALL bonds (including INVALID) for comparison
    result.all_classified_bonds = result.final_bonds;
...
std::vector<HBond> HBondDetector::detect_all_rna_hbonds(const Structure& structure) const {
    std::vector<HBond> all_bonds;

    // Get all residues from all chains
    ...
    for (size_t i = 0; i < residues.size(); ++i) {
        for (size_t j = i + 1; j < residues.size(); ++j) {
            ...
            // Spatial cutoff: skip if residues are too far apart
            // Use a simple C1' distance check
            auto c1_opt1 = res1->find_atom(" C1'");
            auto c1_opt2 = res2->find_atom(" C1'");

            if (!c1_opt1 || !c1_opt2) {
                continue;
            }

            const double distance = (c1_opt1->position() - c1_opt2->position()).length();
            constexpr double SPATIAL_CUTOFF = 20.0; // Angstroms

            if (distance > SPATIAL_CUTOFF) {
                continue;
            }

            // Detect H-bonds between this pair
            auto bonds = detect_all_hbonds_between(*res1, *res2);
...
void HBondDetector::count_potential_hbonds(const Residue& res1, const Residue& res2, int& num_base_hb,
    ...
    for (const auto& a1 : res1.atoms()) {
        for (const auto& a2 : res2.atoms()) {
            ...
            // Legacy pattern: " C5M" or " XD " where X is not H or P and D is a digit
            const bool atom1_is_base = hydrogen_bond::is_base_atom(a1.name());
            const bool atom2_is_base = hydrogen_bond::is_base_atom(a2.name());
            const bool both_base = atom1_is_base && atom2_is_base;
            const bool not_o2prime = (a1.name() != " O2'" && a2.name() != " O2'");

            if (both_base && not_o2prime) {
                if (hydrogen_bond::good_hb_atoms(a1.name(), a2.name(), params_.allowed_elements)) {
                    num_base_hb++;
                }
            }
...
std::vector<HBond> HBondDetector::find_candidate_bonds(const Residue& residue1, const Residue& residue2,
    ...
    for (const auto& atom1 : residue1.atoms()) {
        for (const auto& atom2 : residue2.atoms()) {
            // Check if atoms can form H-bond
            if (!HBondGeometry::are_elements_hbond_compatible(atom1.name(), atom2.name(), params_.allowed_elements)) {
                continue;
            }

            // Skip non-base atoms if requested
            if (base_atoms_only) {
                const bool atom1_is_base = HBondGeometry::is_nucleobase_atom(atom1.name());
                const bool atom2_is_base = HBondGeometry::is_nucleobase_atom(atom2.name());
            ...
            const HBondContext context = HBondGeometry::determine_context(atom1.name(), atom2.name());
...
void HBondDetector::resolve_conflicts_phase3(std::vector<HBond>& bonds) const {
    // Phase 3 in legacy code negates additional bonds for classification
    // Legacy: if (is_not_conflict && has_positive_distance && in_range) negate
    // This causes them to be classified in validate_hbonds
    //
    // In our flow, we classify ALL bonds (not just conflict winners),
    // so we don't need to mark anything for classification here.
    // The post-validation filtering will handle distance-based filtering.
    //
    // NOTE: This is intentionally empty to match legacy behavior where
    // phase 3 marks bonds for classification, not for filtering.
    (void)bonds; // Suppress unused parameter warning
}

void HBondDetector::classify_bonds(std::vector<HBond>& bonds, char base1_type, char base2_type) const {
    // CRITICAL: Only classify bonds that are conflict winners (matches legacy behavior)
    // Legacy only processes bonds with NEGATIVE distance (marked by conflict resolution)
    // In our system, these are IS_CONFLICT_WINNER bonds
    // All other bonds should remain INVALID (type=' ')
    for (auto& bond : bonds) {
        ...
        // Only classify conflict winners (matches legacy: if (hb_dist[k] > 0.0) continue;)
        if (bond.conflict_state != ConflictState::IS_CONFLICT_WINNER) {
            bond.classification = HBondClassification::INVALID;
            continue;
        }

        // Classify using role classifier
        bond.classification = HBondRoleClassifier::classify_bond(base1_type, base2_type, bond.donor_atom_name,
                                                                 bond.acceptor_atom_name);
    }
}
...
void HBondDetector::apply_post_validation_filtering(std::vector<HBond>& bonds) const {
    ...
    // This allows us to include ALL bonds (including INVALID) in JSON output
    ...
    // NOTE: Bonds are NOT removed here - that happens in the caller
}

char HBondDetector::get_base_type_for_hbond(const Residue& residue) {
    // Check one_letter_code first
    const char code = residue.one_letter_code();
    if (code != '?') {
        return code;
    }
