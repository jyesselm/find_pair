/**
 * @file atom_utils.cpp
 * @brief Implementation of atom utilities
 */
...
bool good_hb_atoms(const std::string& atom1, const std::string& atom2, const std::string& hb_atoms) {
    ...
    using constants::hbond_data::HBOND_EXCLUDED_ATOMS;
    bool atom1_in_po = false;
    bool atom2_in_po = false;
    for (const auto& excluded : HBOND_EXCLUDED_ATOMS) {
        if (atom1 == excluded)
            atom1_in_po = true;
        if (atom2 == excluded)
            atom2_in_po = true;
