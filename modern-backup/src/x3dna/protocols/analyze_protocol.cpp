...
/**
 * @brief Verify recalculated frames match original frames
 *
 * @param structure Structure with frames to verify
 * @param original_frames Map of legacy idx -> original frame
 */
void verify_frames_match(const core::Structure& structure,
                         const std::map<int, core::ReferenceFrame>& original_frames) {
    ...
    for (const auto& chain : structure.chains()) {
        for (const auto& residue : chain.residues()) {
            ...
            auto current_opt = residue.reference_frame();
...
void AnalyzeProtocol::recalculate_frames(core::Structure& structure) {
    ...
    if (need_recalculate) {
        ...
        bool is_rna = algorithms::BaseFrameCalculator::detect_rna(structure);
    ...
    verify_frames_match(structure, original_frames);
...
void AnalyzeProtocol::calculate_parameters(core::Structure& /* structure */) {
    ...
    for (size_t i = start_idx; i + 1 < base_pairs_.size(); i += config_.step_size) {
        ...
        // All base pairs have frames (required by BasePair constructor)
        ...
        const auto& frame1 = pair1.frame1();
        const auto& frame2 = pair2.frame1();
