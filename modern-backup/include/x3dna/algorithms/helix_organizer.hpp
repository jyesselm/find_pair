...
private:
    ...
    /** @brief Find pairs within neighbor cutoff distance, sorted by distance */
    [[nodiscard]] std::vector<std::pair<double, size_t>> find_nearby_pairs(
        size_t pair_idx, const std::vector<core::BasePair>& pairs) const;

    /** @brief Legacy special case: try to find neighbor2 indirectly through neighbor1 */
    [[nodiscard]] std::optional<size_t> try_find_indirect_neighbor2(
    ...
    /** @brief Handle normal (non-mixed) direction case in check_strand2 */
    void check_strand2_normal(const std::vector<core::BasePair>& pairs, const BackboneData& backbone,
                              const std::vector<size_t>& pair_order, const HelixSegment& helix,
                              std::vector<bool>& swapped) const;

    /** @brief Handle mixed direction case in check_strand2 */
    void check_strand2_mixed(const std::vector<core::BasePair>& pairs, const BackboneData& backbone,
                             const std::vector<size_t>& pair_order, HelixSegment& helix, std::vector<bool>& swapped,
                             const DirectionCounts& direction) const;

    // ensure_five_to_three helper functions
    /** @brief First pass: check orientation and distance for consecutive pairs */
    void first_pass_strand_check(const std::vector<core::BasePair>& pairs, const BackboneData& backbone,
                                 const std::vector<size_t>& pair_order, const HelixSegment& helix,
                                 std::vector<bool>& swapped) const;

    /** @brief Second pass: re-check WC orientation */
    void second_pass_wc_check(const std::vector<core::BasePair>& pairs, const BackboneData& backbone,
                              const std::vector<size_t>& pair_order, const HelixSegment& helix,
                              std::vector<bool>& swapped) const;
    ...
    /** @brief Check if all items in endpoint's neighbor list are already visited */
    [[nodiscard]] bool should_skip_endpoint(size_t ep, const PairContext& ep_ctx,
                                            const std::vector<bool>& visited) const;

    /** @brief Add endpoint and its neighbors to pair_order, mark as visited */
    void add_endpoint_and_neighbors(size_t ep, const PairContext& ep_ctx, std::vector<size_t>& pair_order,
                                    std::vector<bool>& visited) const;

    /** @brief Traverse helix from current position, adding unvisited neighbors */
    void traverse_helix(const std::vector<PairContext>& context, std::vector<size_t>& pair_order,
                        std::vector<bool>& visited, size_t helix_start) const;

    /** @brief Collect all unvisited pairs into a vector */
    [[nodiscard]] std::vector<size_t> collect_leftover_pairs(const std::vector<bool>& visited, size_t num_pairs) const;
