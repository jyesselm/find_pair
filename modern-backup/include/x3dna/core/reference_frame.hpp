...
public:
     ...
     */
    ReferenceFrame() : rotation_(geometry::Matrix3D::identity()), origin_(geometry::Vector3D(0, 0, 0)), num_ring_atoms_(0) {}
     ...
     * @param num_ring_atoms Number of ring atoms used in fitting (6=pyrimidine, 9=purine)
     */
    ReferenceFrame(const geometry::Matrix3D& rotation, const geometry::Vector3D& origin, size_t num_ring_atoms = 0)
        : rotation_(rotation), origin_(origin), num_ring_atoms_(num_ring_atoms) {}
     ...
     * @param num_ring_atoms Number of ring atoms used in fitting (6=pyrimidine, 9=purine)
     */
    ReferenceFrame(const std::array<double, 9>& rotation_array, const std::array<double, 3>& origin_array, size_t num_ring_atoms = 0)
        : rotation_(geometry::Matrix3D(rotation_array[0], rotation_array[1], rotation_array[2], rotation_array[3],
          ...
          num_ring_atoms_(num_ring_atoms) {}
    ...
    /**
     * @brief Get number of ring atoms used in fitting
     * @return Number of ring atoms (6=pyrimidine, 7-9=purine, 0=unknown)
     */
    [[nodiscard]] size_t num_ring_atoms() const {
        return num_ring_atoms_;
    }

    /**
     * @brief Check if frame was calculated as purine (7+ ring atoms)
     * @return true if purine fit was used
     */
    [[nodiscard]] bool is_purine_fit() const {
        return num_ring_atoms_ >= 7;
    }
...
private:
    ...
    size_t num_ring_atoms_ = 0;   // Number of ring atoms used in fitting (6=pyrimidine, 9=purine)
};
