...
public:
    ...
    [[nodiscard]] bool is_nucleotide() const {
        const char code = one_letter_code();
        // Standard nucleotides
        const bool is_standard = (code == 'A' || code == 'C' || code == 'G' || code == 'T' || code == 'U' ||
                                  code == 'I');
        // Modified nucleotides (lowercase) and Pseudouridine (P)
        const bool is_modified = (code == 'a' || code == 'c' || code == 'g' || code == 't' || code == 'u' ||
                                  code == 'P');
        return is_standard || is_modified;
    }
    ...
    // === New Classification Methods (opt-in, backwards compatible) ===

    /**
     * @brief Get full residue classification
     *
     * Returns a comprehensive classification with molecule type, base type,
     * RNA/DNA distinction, and modification status. Lazy-initialized on first call.
     *
     * @return ResidueClassification with all type information
     */
    [[nodiscard]] const ResidueClassification& classification() const {
        if (!classification_.has_value()) {
            classification_ = ModifiedNucleotideRegistry::classify(trim_name());
        }
     ...
     * @return true if RNA, false otherwise
     */
    [[nodiscard]] bool is_rna() const {
        return classification().is_rna();
    }
    ...
    [[nodiscard]] bool is_dna() const {
        return classification().is_dna();
    }

    /**
     * @brief Check if this is a modified nucleotide
     * @return true if modified (lowercase one-letter code), false if canonical
     */
    [[nodiscard]] bool is_modified() const {
        return classification().is_modified;
    }

    /**
     * @brief Get canonical base code
     * @return Canonical single-letter code ('A', 'C', 'G', 'T', 'U', or '?')
     */
    [[nodiscard]] char canonical_code() const {
        return classification().canonical_code;
    }

    /**
     * @brief Check if this is a water molecule
     * @return true if water (HOH, WAT, H2O)
     */
    [[nodiscard]] bool is_water() const {
        return classification().is_water();
    }

    /**
     * @brief Check if this is an ion
     * @return true if ion (MG, NA, K, etc.)
     */
    [[nodiscard]] bool is_ion() const {
        return classification().is_ion();
    }

    /**
     * @brief Check if this is a ligand
     * @return true if ligand (small molecule, not nucleotide/protein/water/ion)
     */
    [[nodiscard]] bool is_ligand() const {
        return classification().is_ligand();
    }

    /**
     * @brief Check if this is a protein residue (amino acid)
     * @return true if amino acid
     */
    [[nodiscard]] bool is_protein() const {
        return classification().is_protein();
    }
...
private:
    ...
    mutable std::optional<ResidueClassification> classification_; // Lazy-initialized classification
