...
public:
    ...
    [[nodiscard]] bool operator==(const Vector3D& other) const {
        return std::abs(x_ - other.x_) < config::params::EPSILON_VECTOR &&
               std::abs(y_ - other.y_) < config::params::EPSILON_VECTOR &&
               std::abs(z_ - other.z_) < config::params::EPSILON_VECTOR;
    }
    ...
    [[nodiscard]] Vector3D normalized() const {
        double len = length();
        if (len < config::params::EPSILON_VECTOR) {
