...
public:
    ...
    FitResult fit(const std::vector<Vector3D>& points1, const std::vector<Vector3D>& points2) {
        ...
        // Build 4x4 quaternion matrix
        Matrix4D quat_matrix = build_quaternion_matrix(cov);

        // Find largest eigenvalue and corresponding eigenvector (quaternion)
        Vector4D quaternion = find_largest_eigenvector(quat_matrix);
...
private:
     ...
     */
    Matrix4D build_quaternion_matrix(const Matrix3D& covariance) {
        Matrix4D result;

        // Diagonal elements
        double u11 = covariance.at(0, 0), u22 = covariance.at(1, 1), u33 = covariance.at(2, 2);
        result[0][0] = u11 + u22 + u33;
        result[1][1] = u11 - u22 - u33;
        result[2][2] = -u11 + u22 - u33;
        result[3][3] = -u11 - u22 + u33;

        // Off-diagonal elements
        double u12 = covariance.at(0, 1), u21 = covariance.at(1, 0);
        double u13 = covariance.at(0, 2), u31 = covariance.at(2, 0);
        double u23 = covariance.at(1, 2), u32 = covariance.at(2, 1);
     ...
     */
    Vector4D find_largest_eigenvector(const Matrix4D& mat) {
    ...
    Matrix3D quaternion_to_rotation_matrix(const Vector4D& q) {
        // Build quaternion outer product matrix (as in original code)
        Matrix4D outer_product;
        for (size_t i = 0; i < 4; ++i) {
            for (size_t j = 0; j < 4; ++j) {
                outer_product[i][j] = q[i] * q[j];
            }
        }

        // Extract rotation matrix (matching original code)
        Matrix3D rotation;
        rotation.set(0, 0, outer_product[0][0] + outer_product[1][1] - outer_product[2][2] - outer_product[3][3]);
        rotation.set(0, 1, 2.0 * (outer_product[1][2] - outer_product[0][3]));
        rotation.set(0, 2, 2.0 * (outer_product[1][3] + outer_product[0][2]));
        rotation.set(1, 0, 2.0 * (outer_product[2][1] + outer_product[0][3]));
        rotation.set(1, 1, outer_product[0][0] - outer_product[1][1] + outer_product[2][2] - outer_product[3][3]);
        rotation.set(1, 2, 2.0 * (outer_product[2][3] - outer_product[0][1]));
        rotation.set(2, 0, 2.0 * (outer_product[3][1] - outer_product[0][2]));
        rotation.set(2, 1, 2.0 * (outer_product[3][2] + outer_product[0][1]));
        rotation.set(2, 2, outer_product[0][0] - outer_product[1][1] - outer_product[2][2] + outer_product[3][3]);
