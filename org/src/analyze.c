...
typedef struct_args_ana struct_args;
                                   ...
                                   char **Miscs, double **xyz, FILE *fp)
{
    ...
    char BDIR[BUF512], idmsg[BUF512], sidmsg[BUF1K], spdb[BUF512];
    ...
    for (i = 1; i <= ds; i++) {
        ...
        for (j = 1; j <= num_bp; j++) {
            ...
            for (k = 1; k <= 3; k++)
                fprintf(fp, fmt, cxyz[k]);
            for (k = 1; k <= 3; k++)
                fprintf(fp, fmt, nvec[k]);
            fprintf(fp, "\n");
        }
    }
...
static long C1C1_based_frame(long idx, long **chi, double **xyz, double **rotmat)
{
    ...
    if (!c1a || !c1b)
        return FALSE;
    ddxyz(xyz[c1b], xyz[c1a], y_axis);
    for (k = 1; k <= 3; k++)
        z_axis[k] = rotmat[k][3];
    vec_orth(y_axis, z_axis);
                                  ...
                                  long hel_pars, FILE *fp)
{
    ...
    if (hel_pars) {
        fprintf(fp,
                "Simple base-pair helical parameters based on consecutive C1'-C1' vectors\n");
        fprintf(fp,
    ...
    for (i = 1; i < num_bp; i++) {
        ...
        if (C1C1_based_frame(i, chi, xyz, r1) && C1C1_based_frame(ip1, chi, xyz, r2)
            && !bphlx[i]) {
            num++;
            if (hel_pars)
                                    ...
                                    long *WC_info, long *bphlx, double **xyz, FILE *fp)
{
    ...
    num = num_bp - k;
    print_sep(fp, '*', 76);
    fprintf(fp, "The 'simple' parameters are intuitive for non-Watson-Crick base pairs\n");
    fprintf(fp, "and associated base-pair steps (where the above corresponding 3DNA\n");
    fprintf(fp, "parameters often appear cryptic). Note that they are for structural\n");
    fprintf(fp, "*description* only, not to be fed into the 'rebuild' program. See URL\n");
    fprintf(fp, "http://x3dna.org/highlights/details-on-the-simple-base-pair-parameters\n");
    ...
    fprintf(fp, "This structure contains %ld non-Watson-Crick (with leading *) base pair(s)\n",
            num);
    NNy = simple & SIMPLE_BP_LONG_AXIS_RN9_YN1;
    ...
    if (simple & SIMPLE_STEP_HELICAL_PARS)
        simple_step_heli_pars(orien, org, chi, num_bp, bp_seq, WC_info, bphlx, xyz, TRUE, fp);
}

void process_str(char *inpfile, struct_args_ana *args)
{
    ...
    long num_bp, num_residue, str_type = 0;
    ...
    /* Record base pair sequence after it's determined */
    phos = lmatrix(1, ds + 4, 1, num_bp);
    ...
    base_overlap(ds, num_bp, num, num_residue, pair_num, RY, bp_seq, seidx, AtomName, xyz, idx,
                 orien, org, fp);
    nbpm1 = num_bp - 1;
    ...
    json_writer_record_sequence(num_residue, bseq);
    json_writer_record_bp_sequence(num_bp, bp_seq, ds);
...
void analyze_cmdline(int argc, char *argv[], struct_args_ana *args)
{
    long i, j, k;
    set_defaults(args);
    for (i = 1; i < argc; i++) {
    ...
    if (argc == i) {
        ...
        if (is_empty_string(args->torsion)) {
            ...
            calculate_torsions(args->torsion, argv[i]);
        }
    }
}
