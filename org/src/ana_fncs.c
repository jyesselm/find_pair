...
static void check_chi(double chi, char *fmt, char *bstr, char *ostr)
{
    ...
    if (chi > EMPTY_CRITERION) {
        ...
        if (dval_in_range(chi360, 45, 95))
            strcat(ostr, " syn  ");
        else if (in_trans(chi360))
            strcat(ostr, " anti ");
        else
            strcat(ostr, "      ");
    } else
...
void print_header(long ds, long num_bp, long num, char *pdbfile, FILE *fp)
{
    ...
    fprintf(fp, "1. The list of the parameters given below correspond to"
            ...
            " of strand II.\n\n");
    fprintf(fp,
            "2. All angular parameters, except for the phase angle"
                    ...
                    double **nt_torsion)
{
    ...
    for (i = 1; i <= num_residue; i++) {
        ...
        idx[5] = nt_list[i][NT_C4];
        idx[6] = nt_list[i][NT_C3];
        idx[7] = nt_list[i][NT_O3];
        idx[8] = (i < num_residue) ? nt_list[ip1][NT_P] : 0;
        idx[9] = (i < num_residue) ? nt_list[ip1][NT_O5] : 0;
...
static void print_duplex_sugar_conformation(long num_bp, char **bp_seq,
                                            double **sugar_angle, char *bstr, char *fmt,
                                            FILE *fp)
{
    char str[BUF512];
                      ...
                      long **chi, double **xyz, double **nt_bb_torsion, FILE *fp)
{
    ...
    print_sep(fp, '*', 76);
    if (ds == 2)
        print_duplex_torsions(num_bp, pair_num, bp_seq, chi_angle, nt_bb_torsion, bstr, fmt,
                              fp);
    else
...
static double gw_angle(long *idx, double *pvec12, double **xyz)
{
    ...
    ddxyz(xyz[idx[2]], xyz[idx[1]], pvec1);
    ddxyz(xyz[idx[4]], xyz[idx[3]], pvec2);
    vec_norm(pvec1);
    vec_norm(pvec2);
                               ...
                               long *WC_info)
{
    ...
    for (i = 1; i <= num_bp; i++) {
        ...
        k = (bp_seq[0][i] == '-') &&
            dot(&orien[1][j], &orien[2][j]) > 0.0 &&
            dot(&orien[1][j + 3], &orien[2][j + 3]) > 0.0;
        if (k) {
                ...
                long *WC_info, long *str_type, long irna, long **o3p_brk)
{
    ...
    static char *rRingAtom[] = { " C1'", RA_LIST };
    char bp_sym[BUF32], BDIR[BUF512], idmsg[BUF512], sidmsg[BUF1K], spdb[BUF512];
        ...
        if (ib && ib == ik) {
            *str_type = 1;
            for (i = 1; i <= ds; i++)
                for (j = 1; j <= num_bp; j++) {
                    ioffset9 = (j - 1) * 9;
                    negate_xyz(orien[i] + ioffset9);
                    negate_xyz(orien[i] + ioffset9 + 6);
                }
        }
        if (ib && ib != ik)
            *str_type = 2;
        if (ik != ds * num_bp)
            *str_type = *str_type + 10;
    }
    print_sep(fp, '*', 76);
                 ...
                 double **mst_orien, double *mst_org)
{
    ...
    for (i = 1; i <= 3; i++) {
        t1[i] = rot1_h[i][1] + rot2_h[i][1];
        t2[i] = rot1_h[i][2] + rot2_h[i][2];
    }
    vec_norm(t1);
    vec_norm(t2);
...
static void single_helix(long num_bp, char **bp_seq, double **step_par, double **heli_par,
                         double **orien, double **org, FILE *fp, long **pair_num,
                         char **nt_info)
{
    char str[BUF512];
    double **parmtx;
                         ...
                         long istep, long bz, long *str_type, long **pair_num, char **nt_info)
{
    ...
    for (i = 1; i <= num_bp; i++) {
        ...
        fprintf(fp, " %4ld %c%c%c ", i, bp_seq[1][i], bp_seq[0][i], bp_seq[2][i]);
        for (j = 1; j <= 3; j++)
            fprintf(fp, fmt, mfoi[j]);
        for (j = 1; j <= 3; j++)
            fprintf(fp, fmt, mfi[j][3]);
        fprintf(fp, "\n");
              ...
              long *bz_junction, long *z_step)
{
    ...
    if (dot(x1, x2) > 0.0 && dot(z1, z2) > 0.0 && dot(y1_osx, y2) > 0.0 && dot(dorg, z1) < 0.0
        && dot(dorg, z2) < 0.0) {
        for (i = 1; i <= 3; i++) {
            r1[i][1] = -r1[i][1];
            r1[i][3] = -r1[i][3];
    ...
    for (i = 1; i <= num_bp - 1; i++) {
        ...
        for (j = 1; j <= tnum_res1 + tnum_res2; j++) {
            ...
            change_xyz(0, &mst_org[ioffset3], mst, seidx[jr][2] - seidx[jr][1] + 1,
                       xyz_residue);
            pdb_record(seidx[jr][1], seidx[jr][2], &inum, 1, AtomName, ResName, ChainID, ResSeq,
                       xyz_residue, Miscs, fp);
        }
        fprintf(fp, "ENDMDL\n");
    }
                ...
                long p_offset)
{
    ...
    for (i = 1; i <= nbpm1; i++) {
        ...
        if (ip1) {
            ...
            for (j = 1; j <= 3; j++)
                strcat(str, bstr);
        ip2 = (parallel) ? phos[2 + p_offset][i + 1] : phos[2 + p_offset][i];
        if (ip2) {
              ...
              double **xyz, long *WC_info, long *bphlx, long abi, long **chi, FILE *fp)
{
    ...
    print_xyzP(parallel, nbpm1, bp_seq, phos, mst_orien, mst_org, xyz, fchek, str, aveS,
               p_offset);
    sprintf(str,
            "xyz coordinates of O1P atoms w.r.t. the middle helix frame" " of each dimer\n\n");
    ...
    print_xyzP(parallel, nbpm1, bp_seq, phos, mst_orien, mst_org, xyz, fchek, str, aveS,
               p_offset);
    sprintf(str,
            "xyz coordinates of O2P atoms w.r.t. the middle helix frame" " of each dimer\n\n");
    strcat(str, "    step      xIH     yIH     zIH     xIIH    yIIH    zIIH\n");
    print_xyzP(parallel, nbpm1, bp_seq, phos, mst_orienH, mst_orgH, xyz, fchek,
               str, aveH, p_offset);
    p_offset = 0;
    ...
    print_xyzP(parallel, nbpm1, bp_seq, phos, mst_orien, mst_org, xyz, fchek, str, aveS,
               p_offset);
    sprintf(str,
            "xyz coordinates of P atoms w.r.t. the middle helix frame" " of each dimer\n\n");
    strcat(str, "    step      xIH     yIH     zIH     xIIH    yIIH    zIIH\n");
    print_xyzP(parallel, nbpm1, bp_seq, phos, mst_orienH, mst_orgH, xyz, fchek,
               str, aveH, p_offset);
    close_file(fchek);
    ...
    for (i = 1; i <= nbpm1; i++) {
        ...
        if (ip && !bphlx[i]) {
            ...
            strcat(step_info[i], str);
            if (WC_info[i] && WC_info[i + 1] &&
                dval_in_range(twist_rise[i][1], 10.0, 60.0) &&
                dval_in_range(twist_rise[i][2], 2.5, 5.5) &&
                dval_in_range(aveS[i][1], -5.0, -0.5) &&
                dval_in_range(aveS[i][2], 7.5, 10.0) &&
                dval_in_range(aveS[i][3], -2.0, 3.5) &&
                dval_in_range(aveH[i][1], -11.5, 2.5) &&
                dval_in_range(aveH[i][2], 1.5, 10.0) && dval_in_range(aveH[i][3], -3.0, 9.0)) {
                if (aveS[i][3] >= 1.5)
                    strABT[i] = 1;
                else if (aveH[i][3] >= 4.0)
                    strABT[i] = 3;
                else if (aveS[i][3] <= 0.5 && aveH[i][1] < 0.5)
                    strABT[i] = 2;
                if (abi)
                  ...
                  long **phos, long **chi, double **xyz, long *bphlx, FILE *fp)
{
    ...
    o4_radius = dmatrix(1, ds, 1, nbpm1);
    c1_radius = dmatrix(1, ds, 1, nbpm1);
    for (i = 1; i <= ds; i++)
        for (j = 1; j <= nbpm1; j++) {
            refs_i_j(j, j + 1, orien[i], org[i], r1, o1, r2, o2);
...
static void get_poc_radius(long ds, long num_bp, long **phos, long **chi, double **xyz,
                           double *haxis, double *hstart, double *hend, double *rave,
                           double *rstd)
{
                            ...
                            long **ring_atom, double **xyz)
{
    ...
    n1 = ratom_xyz(ring_atom[r1], 0, xyz, oave, oxyz1);
...
void get_zoave(long istep, long ds, double **orien, double **org, double *oave, double *zave)
{
    ...
    ioffset3 = (istep - 1) * 3;
    ioffset9 = (istep - 1) * 9;
    d = dot(&orien[1][ioffset9 + 6], &orien[1][ioffset9 + 15]);
    ...
    if (ds == 2) {
        ...
        d = dot(&orien[1][ioffset9 + 6], &orien[ds][ioffset9 + 6]);
        for (j = 1; j <= 3; j++)
            zave[j] += (d > 0.0) ? orien[ds][ioffset9 + 6 + j] : -orien[ds][ioffset9 + 6 + j];
        d = dot(&orien[1][ioffset9 + 6], &orien[ds][ioffset9 + 15]);
        for (j = 1; j <= 3; j++)
...
void get_cntatom(long *ringlist, long **connect, long *idx)
{
    ...
    for (i = 1; i <= ra_num; i++) {
        ...
        for (j = 1; j <= connect[id][7]; j++) {
            ...
            if (idx[ic] == 3)
                continue;
            if (!lval_in_set(ic, 1, ra_num, ringlist)) {
                ix = 1;
                break;
            }
        }
...
static double pia_inter(point *a, long na, point *b, long nb)
{
    ...
    long j, k, o;
    vertex ipa[MNPOLY + 1], ipb[MNPOLY + 1];
    if (na < 3 || nb < 3)
        return 0;
    minx = miny = XBIG;
    maxx = maxy = -XBIG;
...
static void check_cehs_step_frames(double **r1, double *o1, double **r2, double *o2)
{
    double sum, dd[4];
    long i;
    ddxyz(o1, o2, dd);
               ...
               double *bp_org, FILE *fp)
{
    ...
    for (i = 1; i <= ds; i++) {
        for (j = 1; j <= num_bp; j++) {
            ...
            cross(orien[i] + ioffset + 3, orien[i] + ioffset + 6, orien[i] + ioffset);
        }
    }
    for (i = 1; i <= num_bp; i++) {
        ...
        cross(bp_orien + ioffset + 3, bp_orien + ioffset + 6, bp_orien + ioffset);
    }
    r1 = dmatrix(1, 3, 1, 3);
    r2 = dmatrix(1, 3, 1, 3);
    mfi = dmatrix(1, 3, 1, 3);
    ...
    prt_stepstr(step_str, num_step, bphlx, 0, step_par, fp);
...
void my_curves(double **rot1, double *org1, double **rot2, double *org2, double *pars)
{
    ...
    cross(n, d, f);
    ddxyz(org1, q, temp);
    dl = dot(n, temp) / dot(n, l1);
    for (i = 1; i <= 3; i++)
        pl[i] = org1[i] + dl * l1[i];
    ddxyz(q, org2, temp);
    du = dot(n, temp) / dot(n, l2);
    for (i = 1; i <= 3; i++)
        pu[i] = org2[i] - du * l2[i];
    pars[3] = dl + du;
    ddxyz(pl, pu, temp);
...
void sgl_helix(double **rot1, double **rot2, double *rot_ang, double *rot_hlx)
{
    ...
    multi_matrix(temp, 3, 3, rot2, 3, 3, R);
    for (i = 1; i <= 3; i++)
        dsum += R[i][i];
    tcos = 0.5 * (dsum - 1.0);
