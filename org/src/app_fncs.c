...
long tag_match(char *prefix, char *line, char *tag)
{
    char btag[BUF512];
    sprintf(btag, "<%s%s", prefix, tag);
    return str_pmatch(line, btag) && !strstr(line, "/>");
}
...
long set_switch_default_true(char *option)
{
    ...
    lowerstr(str);
    if (str_pmatch(str, "of") || str_pmatch(str, "0") ||
...
void define_frame_by_3atoms(long num, double **xyz, double **refmat)
{
    ...
    if (num < 3)
        return;
    cpxyz(xyz[1], refmat[4]);
    ddxyz(xyz[1], xyz[2], tmpx);
    for (i = 3; i <= num; i++) {
        ddxyz(xyz[1], xyz[i], tmpy);
        d = magang(tmpx, tmpy);
        if (dval_in_range(d, 5, 175))
            break;
    }
    if (i > num) {
    ...
    x_y_z_2_mtx(tmpx, tmpy, tmpz, refmat);
}
                ...
                char **Miscs, double **xyz, char *BDIR, double **orien, double **org)
{
    ...
    char idmsg[BUF512], sidmsg[BUF1K], spdb[BUF512];
...
{
    double d;
    d = distance_ab(o3_p, i, j, 4, 8);
    if (dval_in_range(d, 0.0, O3P_UPPER))
        return 1L;
    d = distance_ab(o3_p, j, i, 4, 8);
    if (dval_in_range(d, 0.0, O3P_UPPER))
        return -1L;
    return 0L;
}
                   ...
                   double **xyz, double **orien, double **org)
{
    ...
    long i, ioffset, j, nmatch, pnum = 6;
    long idx[7], *sResSeq;
    sprintf(filename, "%s%s", BDIR, SNAP_PEP_PDB);
    fprintf(stderr, " ...... reading file: %s ...... \n", filename);
    sAtomName = cmatrix(1, pnum, 0, 4);
    ...
    for (i = 1; i < num_residue; i++) {
        org[i][4] = -1.0;
        if (res_type[i] != -1 || res_type[i + 1] != -1)
            continue;
        ioffset = (i - 1) * 4;
        idx[1] = mchain[ioffset + 2];
        idx[2] = mchain[ioffset + 3];
        idx[3] = mchain[ioffset + 4];
        ioffset += 4;
        idx[4] = mchain[ioffset + 1];
        idx[5] = 0;
        idx[6] = mchain[ioffset + 2];
        if (!idx[2] || !idx[4])
            continue;
        if (p1p2_dist(xyz[idx[2]], xyz[idx[4]]) > 2.0)
            continue;
        nmatch = 0;
        for (j = 1; j <= pnum; j++) {
            if (idx[j]) {
...
{
    Gvars.VERBOSE = FALSE;
    Gvars.CHAIN_CASE = TRUE;
    Gvars.ALL_MODEL = FALSE;
    Gvars.DEBUG = FALSE;
    Gvars.PROGNAME = pgname;
    Gvars.ATTACH_RESIDUE = TRUE;
    Gvars.THREE_LETTER_NTS = FALSE;
    Gvars.PDBV3 = TRUE;
    Gvars.ORIGINAL_COORDINATE = FALSE;
    Gvars.OCCUPANCY = FALSE;
    Gvars.HEADER = TRUE;
    Gvars.mmcif = FALSE;
    Gvars.NT_CUTOFF = 0.2618;
    ...
    Gvars.NUM_ELE = sizeof ptable / sizeof ptable[0] - 1;
                   ...
                   double **xyz, char *BDIR, double **orien, double **org)
{
    ...
    for (i = 1; i <= num_residue; i++) {
        ...
        idx[4] = find_1st_atom(" CB ", AtomName, ib, ie, "");
        org[i][4] = -1.0;
        nmatch = 0;
        for (j = 1; j <= pnum; j++) {
            if (idx[j]) {
...
void residue_strid(char chain_id, long res_seq, char *misc, char *rname, char *idmsg)
{
    ...
    modelNum[nlen] = '\0';
    model_lval = cvt2long(modelNum);
    if (iCode == ' ')
        iCode = '.';
    convert_resNameSpace(rname, '_', newName);
    if (model_lval == 0)
...
void get_planarFrame(char *aa, long pnum, char *planar_atoms[], char *BDIR, char *idmsg,
                     ...
                     double *org_i)
{
    char spdb[BUF512], sidmsg[BUF1K], *sChainID, **sAtomName, **sResName;
                  ...
                  double **xyz, double *orien_i, double *org_i)
{
    ...
    get_planarFrame("arg", pnum, planar_atoms, BDIR, idmsg, ib, ie, AtomName, xyz, orien_i,
                    org_i);
}
                  ...
                  double **xyz, double *orien_i, double *org_i)
{
    ...
    long pnum = sizeof planar_atoms / sizeof planar_atoms[0] - 1;
    get_planarFrame("phe", pnum, planar_atoms, BDIR, idmsg, ib, ie, AtomName, xyz, orien_i,
                    org_i);
}
                  ...
                  double **xyz, double *orien_i, double *org_i)
{
    ...
    long pnum = sizeof planar_atoms / sizeof planar_atoms[0] - 1;
    get_planarFrame("tyr", pnum, planar_atoms, BDIR, idmsg, ib, ie, AtomName, xyz, orien_i,
                    org_i);
}
...
{
    static char *planar_atoms[] =
        { "XXXX", " CB ", " CG ", " CD1", " CD2", " NE1", " CE2", " CE3",
        " CZ2", " CZ3", " CH2"
    };
    long pnum = sizeof planar_atoms / sizeof planar_atoms[0] - 1;
    get_planarFrame("trp", pnum, planar_atoms, BDIR, idmsg, ib, ie, AtomName, xyz, orien_i,
                    org_i);
}
                  ...
                  double **xyz, double *orien_i, double *org_i)
{
    ...
    long pnum = sizeof planar_atoms / sizeof planar_atoms[0] - 1;
    get_planarFrame("his", pnum, planar_atoms, BDIR, idmsg, ib, ie, AtomName, xyz, orien_i,
                    org_i);
}
                  ...
                  double **xyz, double *orien_i, double *org_i)
{
    ...
    if (!Gvars.VERBOSE)
        strcpy(idmsg, "");
    get_planarFrame("lys", pnum, planar_atoms, BDIR, idmsg, ib, ie, AtomName, xyz, orien_i,
                    org_i);
}
                  ...
                  double **xyz, double *orien_i, double *org_i)
{
    ...
    long pnum = sizeof planar_atoms / sizeof planar_atoms[0] - 1;
    get_planarFrame("asn", pnum, planar_atoms, BDIR, idmsg, ib, ie, AtomName, xyz, orien_i,
                    org_i);
}
                  ...
                  double **xyz, double *orien_i, double *org_i)
{
    ...
    long pnum = sizeof planar_atoms / sizeof planar_atoms[0] - 1;
    get_planarFrame("gln", pnum, planar_atoms, BDIR, idmsg, ib, ie, AtomName, xyz, orien_i,
                    org_i);
}
                  ...
                  double **xyz, double *orien_i, double *org_i)
{
    ...
    long pnum = sizeof planar_atoms / sizeof planar_atoms[0] - 1;
    get_planarFrame("asp", pnum, planar_atoms, BDIR, idmsg, ib, ie, AtomName, xyz, orien_i,
                    org_i);
}
                  ...
                  double **xyz, double *orien_i, double *org_i)
{
    ...
    long pnum = sizeof planar_atoms / sizeof planar_atoms[0] - 1;
    get_planarFrame("glu", pnum, planar_atoms, BDIR, idmsg, ib, ie, AtomName, xyz, orien_i,
                    org_i);
}
                  ...
                  double *orgP, double **rotP)
{
    ...
    sgl_helix(rot1, rot2, rot_dist, temp);
    ddxyz(org1, org2, temp);
    multi_vec_matrix(temp, 3, rot1, 3, 3, orgP);
    *trs_dist = veclen(temp);
    if (orgP[3] < 0.0)
        *trs_dist = -*trs_dist;
...
void cleanup_files(long renew, long cleanup)
{
    ...
    char filename[BUF1K], suffix[BUF512];
