...
class AggregateStats:
    ...
    Attributes:
        ...
        dssr_questionable: Pairs flagged as likely DSSR errors.
        ...
        return {
            ...
            "rmsd_gap_distribution": dict(
                sorted(
                    ...
                    key=lambda x: _sort_rmsd_bin(x[0]),
                )
            ),
            "sequence_breakdown": {
                seq: {
...
def save_pdb_report(report: PDBReport, output_path: Path) -> None:
    """Save single PDB report to JSON."""
    with open(output_path, "w") as f:
        json.dump(report.to_dict(), f, indent=2)


def save_aggregate_report(stats: AggregateStats, output_path: Path) -> None:
