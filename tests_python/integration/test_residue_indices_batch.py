...
from x3dna_json_compare.residue_indices_comparison import compare_residue_indices
...
def compare_residue_indices_json(legacy_file: Path, modern_file: Path) -> Tuple[bool, Dict]:
    """Compare legacy and modern residue_indices JSON files using x3dna_json_compare."""
    try:
        # Load JSON files
        ...
        # Normalize to lists of records
        if isinstance(legacy_data, list):
            legacy_records = legacy_data
        elif isinstance(legacy_data, dict):
            if legacy_data.get("type") == "residue_indices":
                legacy_records = [legacy_data]
            elif "calculations" in legacy_data:
                legacy_records = [r for r in legacy_data.get("calculations", []) 
                                if r.get("type") == "residue_indices"]
            else:
                legacy_records = [legacy_data]
        else:
        ...
        if isinstance(modern_data, list):
            ...
            if modern_data.get("type") == "residue_indices":
                modern_records = [modern_data]
            elif "calculations" in modern_data:
                modern_records = [r for r in modern_data.get("calculations", []) 
                                if r.get("type") == "residue_indices"]
            else:
                modern_records = [modern_data]
        else:
...
def main():
    ...
    if valid_pdb_ids:
        print(f"  (Filtered from {len(all_pdb_files)} total using valid_pdbs_fast.json)")
