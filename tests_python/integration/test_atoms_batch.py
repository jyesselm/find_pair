...
from x3dna_json_compare.atom_comparison import compare_atoms
...
def generate_legacy_atoms(pdb_id: str, pdb_file: Path, output_dir: Path, 
    ...
    # Need to regenerate - use shared utility
    ok, msg = generate_legacy_json(
        pdb_id, pdb_file, output_dir, legacy_exe, project_root,
        record_types=["pdb_atoms"]
    )
    
    # Copy from generated location to expected location
    if ok:
        generated_file = legacy_output / "pdb_atoms" / f"{pdb_id}.json"
        if generated_file.exists():
            shutil.copy2(generated_file, legacy_output / f"{pdb_id}.json")
...
def compare_atoms_json(legacy_file: Path, modern_file: Path) -> Tuple[bool, Dict]:
    """Compare legacy and modern atoms JSON using x3dna_json_compare."""
    try:
        ...
        if isinstance(legacy_data, list):
            if len(legacy_data) > 0 and isinstance(legacy_data[0], dict):
                if "atoms" in legacy_data[0] or legacy_data[0].get("type") == "pdb_atoms":
        ...
        comparison_result = compare_atoms(legacy_atoms, modern_atoms)
        
        if (comparison_result.missing_in_modern or 
            comparison_result.extra_in_modern or 
            comparison_result.mismatched_fields):
            errors = []
            if comparison_result.missing_in_modern:
                errors.append(f"{len(comparison_result.missing_in_modern)} missing in modern")
            if comparison_result.extra_in_modern:
                errors.append(f"{len(comparison_result.extra_in_modern)} extra in modern")
            if comparison_result.mismatched_fields:
                errors.append(f"{len(comparison_result.mismatched_fields)} field mismatches")
        ...
        return True, {"matched": comparison_result.common_count}
...
def main():
    ...
    try:
        pdb_list = load_valid_pdbs_fast(project_root)
        print(f"  Using valid_pdbs_fast.json ({len(pdb_list)} PDBs)")
        valid_pdb_ids = set(pdb_list)
