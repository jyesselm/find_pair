...
int main(int argc, char* argv[]) {
    ...
    for (int idx : target_indices) {
        ...
        std::cout << "Found residue " << idx << ": " << it->second->name() 
                  << " (Chain " << it->second->chain_id() 
    ...
    for (auto* residue : target_residues) {
        ...
        if (frame_result.is_valid) {
            std::cout << "Frame calculated for " << residue->name() 
                      << " (legacy_idx " << residue->atoms()[0].legacy_residue_idx() << ")\n";
            std::cout << "  Origin: [" << frame_result.frame.origin().x() << ", "
                      << frame_result.frame.origin().y() << ", "
    ...
    for (size_t i = 0; i < target_residues.size(); ++i) {
        ...
        writer.record_base_frame_calc(
            record_idx,
            base_type,
            frame_result.template_file,
            frame_result.rms_fit,
            frame_result.matched_atoms,
            residue->name(),
            residue->chain_id(),
            residue->seq_num(),
            residue->insertion()
        ...
        writer.record_ls_fitting(
            record_idx,
            frame_result.num_matched,
            frame_result.rms_fit,
            frame_result.rotation_matrix,
            frame_result.translation,
            residue->name(),
            residue->chain_id(),
            residue->seq_num(),
            residue->insertion()
        ...
        frame_calc_record["frame_origin"] = json::array({
            frame_result.frame.origin().x(),
            frame_result.frame.origin().y(),
            frame_result.frame.origin().z()
        });
        
        auto rot = frame_result.frame.rotation();
        frame_calc_record["rotation_matrix"] = json::array({
            json::array({rot.at(0, 0), rot.at(0, 1), rot.at(0, 2)}),
