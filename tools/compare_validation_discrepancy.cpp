...
void print_comparison(const LegacyValidationResult& legacy, const ValidationResult& modern, 
    ...
    std::cout << "FRAMES:\n";
    std::cout << "  Frame 1 origin: [" << frame1.origin().x() << ", " 
              << frame1.origin().y() << ", " << frame1.origin().z() << "]\n";
    std::cout << "  Frame 2 origin: [" << frame2.origin().x() << ", " 
              << frame2.origin().y() << ", " << frame2.origin().z() << "]\n";
    ...
    std::cout << "  Modern:  is_valid=" << (modern.is_valid ? "YES" : "NO") 
              << ", quality=" << modern.quality_score << ", bp_type=" << static_cast<int>(modern.bp_type) << "\n";
    ...
    if (legacy.is_valid != modern.is_valid) {
        ...
        if (!legacy.is_valid && modern.is_valid) {
            ...
            if (!legacy.distance_check) std::cout << "    - distance_check FAILED\n";
            if (!legacy.d_v_check) std::cout << "    - d_v_check FAILED\n";
            if (!legacy.plane_angle_check) std::cout << "    - plane_angle_check FAILED\n";
            if (!legacy.dNN_check) std::cout << "    - dNN_check FAILED\n";
        } else {
...
int main(int argc, char* argv[]) {
    if (argc != 5) {
        ...
        std::cerr << "Example: " << argv[0] << " data/pdb/6CAQ.pdb data/json_legacy/pair_validation/6CAQ.json 980 997\n";
        return 1;
    }
