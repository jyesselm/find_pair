...
int main(int argc, char* argv[]) {
    ...
    std::cout << "Residue 1: " << res1->name() << " (chain " << res1->chain_id() 
              << ", seq " << res1->seq_num() << ")\n";
    std::cout << "Residue 2: " << res2->name() << " (chain " << res2->chain_id() 
              << ", seq " << res2->seq_num() << ")\n";
    ...
    // Legacy typically includes: base atoms + backbone atoms (P, O1P, O2P, O5', C5', C4', O4', C3', O3', C2', O2', C1')
    // For modified nucleotides, it might include additional atoms
    ...
    std::set<std::string> hbond_atoms = {"N1", "N2", "N3", "N4", "N6", "N7", "N9", 
                                          "O2", "O4", "O6", "O4'", "O2'", "O3'", "O5'",
                                          "O1P", "O2P"};
    ...
    for (const auto& atom : modern_atoms1) {
        if (atom.find(" O4 ") != std::string::npos) has_o4 = true;
        if (atom.find("O4'") != std::string::npos) has_o4prime = true;
        if (atom.find(" N6 ") != std::string::npos) has_n6 = true;
    }
