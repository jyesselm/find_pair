 ...
 *   record_type: frame_calc, base_frame_calc, pair_validation, find_bestpair_selection (default:
 * all)
 */
...
bool compare_indices(const json& legacy_records, const json& modern_records,
    ...
    if (record_type == "frame_calc" || record_type == "base_frame_calc") {
        ...
        for (const auto& [idx, _] : legacy_map)
            all_indices.insert(idx);
        for (const auto& [idx, _] : modern_map)
            all_indices.insert(idx);
        ...
        if (only_legacy > 0)
            std::cout << "Only in legacy: " << only_legacy << "\n";
        if (only_modern > 0)
            std::cout << "Only in modern: " << only_modern << "\n";
...
int main(int argc, char* argv[]) {
    if (argc < 2) {
        ...
        std::cerr << "  record_type: frame_calc, base_frame_calc, pair_validation, "
