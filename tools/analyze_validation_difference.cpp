...
void print_validation_result(const ValidationResult& result,
    ...
    std::cout << "  d_v_check: " << (result.d_v_check ? "PASS" : "FAIL")
              << " [" << params.min_dv << " <= " << result.d_v << " <= " << params.max_dv << "]\n";
    ...
    std::cout << "  dNN_check: " << (result.dNN_check ? "PASS" : "FAIL")
              << " [" << params.min_dNN << " <= " << result.dNN << " <= " << params.max_dNN << "]\n";
...
int main(int argc, char* argv[]) {
    ...
    std::cout << "  Residue " << idx1 << ": " << mutable_res1->name() 
              << " (chain " << mutable_res1->chain_id() << ")\n";
    std::cout << "  Residue " << idx2 << ": " << mutable_res2->name() 
              << " (chain " << mutable_res2->chain_id() << ")\n";
    ...
    if (!frame1_result.is_valid || !frame2_result.is_valid) {
        ...
        if (!frame1_result.is_valid) std::cerr << "  Frame 1 (residue " << idx1 << ") failed\n";
        if (!frame2_result.is_valid) std::cerr << "  Frame 2 (residue " << idx2 << ") failed\n";
        return 1;
    }
    ...
    if (!passes_cdns) {
        ...
        if (!result.distance_check) std::cout << "  - dorg check failed\n";
        if (!result.d_v_check) std::cout << "  - d_v check failed\n";
        if (!result.plane_angle_check) std::cout << "  - plane_angle check failed\n";
        if (!result.dNN_check) std::cout << "  - dNN check failed\n";
    } else {
