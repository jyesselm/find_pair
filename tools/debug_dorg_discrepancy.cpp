...
int main(int argc, char* argv[]) {
    ...
    if (res1->reference_frame().has_value()) {
        ...
        for (int i = 0; i < 3; ++i) {
            std::cout << "    [" << frame1.rotation().at(i, 0) << ", " 
                      << frame1.rotation().at(i, 1) << ", " 
    ...
    if (res2->reference_frame().has_value()) {
        ...
        std::cout << "  Rotation matrix:\n";
        for (int i = 0; i < 3; ++i) {
            std::cout << "    [" << frame2.rotation().at(i, 0) << ", " 
                      << frame2.rotation().at(i, 1) << ", " 
    ...
    if (res1->reference_frame().has_value() && res2->reference_frame().has_value()) {
        ...
        std::cout << "Origin 1: [" << frame1.origin().x() << ", " 
                  << frame1.origin().y() << ", " << frame1.origin().z() << "]\n";
        std::cout << "Origin 2: [" << frame2.origin().x() << ", " 
    ...
    if (frame_result1.is_valid && frame_result2.is_valid) {
        ...
        if (std::abs(dorg_from_results - result.dorg) > 0.01) {
            ...
            std::cout << "This suggests the frames stored on residues are different from frame_results.\n";
        }
    }
