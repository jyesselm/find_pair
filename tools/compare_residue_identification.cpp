            ...
            if (calc.contains("type")) {
                ...
                if (type == "base_frame_calc" || type == "frame_calc" || type == "ref_frame") {
                    if (calc.contains("chain_id") && calc.contains("residue_seq") && 
                        ...
                        } else if (name_upper.find("C") != std::string::npos || name_upper == "  C" || name_upper == " DC") {
                            info.residue_type = ResidueType::CYTOSINE;
                        } else if (name_upper.find("G") != std::string::npos || name_upper == "  G" || name_upper == " DG") {
                            info.residue_type = ResidueType::GUANINE;
                        } else if (name_upper.find("T") != std::string::npos || name_upper == "  T" || name_upper == " DT") {
                            info.residue_type = ResidueType::THYMINE;
                        } else if (name_upper.find("U") != std::string::npos || name_upper == "  U" || name_upper == " DU") {
...
void print_residue_comparison(const std::vector<ResidueInfo>& modern, 
                             const std::vector<ResidueInfo>& legacy,
    ...
    for (const auto& res : modern) {
        if (res.is_nucleotide) modern_nucleotides++;
    }
    
    for (const auto& res : legacy) {
        if (res.is_nucleotide) legacy_nucleotides++;
    }
