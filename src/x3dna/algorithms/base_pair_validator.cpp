...
std::vector<core::hydrogen_bond> BasePairValidator::find_hydrogen_bonds(const Residue& res1,
    ...
    for (const auto& atom1 : res1.atoms()) {
        for (const auto& atom2 : res2.atoms()) {
            ...
            if (good_hb_atoms(atom1.name(), atom2.name())) {
                ...
                hbond.distance = dist;
                hbond.type = '-'; // Standard H-bond (will be validated later)
    ...
    // If the same atom has multiple H-bonds, keep only the shortest one
    std::vector<core::hydrogen_bond> resolved_hbonds;
    std::vector<bool> keep(all_hbonds.size(), true);

    // For each H-bond, check if there's a shorter one involving the same atom
    for (size_t i = 0; i < all_hbonds.size(); ++i) {
        if (!keep[i]) {
            continue;
        }

        const auto& hbond_i = all_hbonds[i];

        // Check if atom1 (donor) has a shorter H-bond
        for (size_t j = 0; j < all_hbonds.size(); ++j) {
            if (i == j || !keep[j]) {
                continue;
            }
            const auto& hbond_j = all_hbonds[j];
        ...
        // Check if atom2 (acceptor) has a shorter H-bond
        for (size_t j = 0; j < all_hbonds.size(); ++j) {
            if (i == j || !keep[j]) {
                continue;
            }
    ...
    // Collect resolved H-bonds
    for (size_t i = 0; i < all_hbonds.size(); ++i) {
        if (keep[i]) {
            resolved_hbonds.push_back(all_hbonds[i]);
        }
    }

    // Step 3: Validate H-bonds (matches legacy validate_hbonds)
    // Legacy validate_hbonds filters:
    // 1. Determines H-bond type using donor_acceptor
    // 2. Filters H-bonds with distance > 3.6
    // 3. Filters H-bonds with type '*' and linkage type != 18 and distance not in [2.6, 3.2]
    // 4. Only keeps H-bonds with hb_type != ' '
    std::vector<core::hydrogen_bond> validated_hbonds;

    // Get base types for donor_acceptor
    char base1 = res1.one_letter_code();
    char base2 = res2.one_letter_code();

    // Count "good" H-bonds (type == '-' and distance in [2.5, 3.5])
    int num_good_hb = 0;
    for (const auto& hbond : resolved_hbonds) {
        // Determine H-bond type using donor_acceptor
        char hb_type = donor_acceptor(base1, base2, hbond.donor_atom, hbond.acceptor_atom);

        // Count good H-bonds (matches legacy: hb_type == '-' && distance in [2.5, 3.5])
        if (hb_type == '-' && hbond.distance >= 2.5 && hbond.distance <= 3.5) {
            num_good_hb++;
        }
    }

    // Apply filtering (matches legacy validate_hbonds logic)
    // Legacy always processes all H-bonds, but only filters if there are good ones
    for (const auto& hbond : resolved_hbonds) {
        // Determine H-bond type
        char hb_type = donor_acceptor(base1, base2, hbond.donor_atom, hbond.acceptor_atom);

        // Filter out invalid H-bonds (hb_type == ' ')
        if (hb_type == ' ') {
            continue;
        }
        ...
        // If there are good H-bonds, apply additional filtering
        // Legacy: filters non-standard H-bonds with distance outside [2.6, 3.2]
        // Note: Linkage type checking (lkg_type != 18) is not yet implemented
        if (num_good_hb > 0) {
            // Filter out non-standard H-bonds with distance outside [2.6, 3.2]
            if (hb_type == '*' && (hbond.distance < 2.6 || hbond.distance > 3.2)) {
                continue;
            }
        }

        // Valid H-bond - keep it
        core::hydrogen_bond valid_hbond = hbond;
        valid_hbond.type = hb_type;
        validated_hbonds.push_back(valid_hbond);
    }

    return validated_hbonds;
}
