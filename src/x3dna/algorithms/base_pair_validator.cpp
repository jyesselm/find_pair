...
ValidationResult BasePairValidator::validate(const Residue& res1, const Residue& res2) const {
    ...
    if (cdns) {
        ...
        // Find ALL H-bonds (including invalid ones) - matches legacy get_hbond_ij behavior
        // Legacy records ALL H-bonds to JSON in get_hbond_ij (called from calculate_more_bppars)
        // This should be called for ALL pairs that pass hbond_check, regardless of final validity
        // But for now, only calculate if hbond_check passes (reached calculate_more_bppars equivalent)
...
std::vector<core::hydrogen_bond> BasePairValidator::find_hydrogen_bonds(const Residue& res1,
    ...
    double hb_dist2 = 0.0; // Legacy default is 0.0, which means Phase 3 never marks additional conflicts
