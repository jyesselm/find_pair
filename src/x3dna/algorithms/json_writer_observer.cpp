...
void JsonWriterObserver::on_pair_validated(int legacy_idx1, int legacy_idx2, const core::Residue& res1,
                                           const core::Residue& res2, const ValidationResult& result, int bp_type_id) {
    ...
    if (passes_cdns && legacy_idx1 < legacy_idx2) {
        ...
        std::array<double, 5> rtn_val = {result.dorg, result.d_v, result.plane_angle, result.dNN, result.quality_score};

        // Record validation results (only for valid pairs to match legacy behavior)
        if (result.is_valid) {
            writer_.record_pair_validation(base_i, base_j, result.is_valid, bp_type_id, result.dir_x, result.dir_y,
                                           result.dir_z, rtn_val, params_);
        }
    }

    // Record distance checks only for valid pairs when i < j
    if (result.is_valid && legacy_idx1 < legacy_idx2) {
        writer_.record_distance_checks(base_i, base_j, result.dorg, result.dNN, result.plane_angle, result.d_v,
                                       result.overlap_area);
    }
...
void JsonWriterObserver::on_best_partner_candidates(int legacy_idx, const std::vector<BestPartnerCandidate>& candidates,
                                                    int best_partner_idx, double best_score) {
...
void JsonWriterObserver::on_mutual_best_check(int legacy_idx1, int legacy_idx2, int best_j_for_i, int best_i_for_j,
                                              bool is_mutual, bool was_selected) {
    writer_.record_mutual_best_decision(legacy_idx1, legacy_idx2, best_j_for_i, best_i_for_j, is_mutual, was_selected);
}

void JsonWriterObserver::on_iteration_complete(int iteration_num,
                                               const std::vector<std::pair<int, int>>& pairs_this_iteration,
                                               const std::vector<bool>& matched_indices, size_t total_matched) {
    int max_legacy_idx = static_cast<int>(matched_indices.size() - 1);
    writer_.record_iteration_state(iteration_num, total_matched, max_legacy_idx, matched_indices, pairs_this_iteration);
}

void JsonWriterObserver::on_selection_complete(const std::vector<std::pair<int, int>>& selected_pairs) {
...
void JsonWriterObserver::on_pairs_finalized(const std::vector<core::BasePair>& base_pairs) {
