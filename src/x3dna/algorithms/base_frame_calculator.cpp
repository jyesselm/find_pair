...
FrameCalculationResult BaseFrameCalculator::calculate_frame_impl(const core::poly::IResidue& residue) const {
    ...
    // Determine residue type from nucleotide properties
    core::ResidueType residue_type = core::ResidueType::UNKNOWN;
    if (nucleotide->is_purine()) {
        residue_type = (one_letter == 'G' || one_letter == 'g')
            ? core::ResidueType::GUANINE
            : core::ResidueType::ADENINE;
    } else if (nucleotide->is_pyrimidine()) {
        if (one_letter == 'C' || one_letter == 'c') {
            residue_type = core::ResidueType::CYTOSINE;
        } else if (one_letter == 'T' || one_letter == 't') {
            residue_type = core::ResidueType::THYMINE;
        } else if (one_letter == 'P' || one_letter == 'p') {
    ...
    bool should_check_rings = (residue_type == core::ResidueType::UNKNOWN || needs_rmsd_check);

    if (should_check_rings) {
        auto [ring_count, has_purine] = count_ring_atoms_poly(residue);
        has_ring_atoms = (ring_count >= 3);
        has_purine_atoms = has_purine || detect_purine_atoms_poly(residue);
    } else {
        has_ring_atoms = true;
        has_purine_atoms = nucleotide->is_purine();
    }
    ...
    if (has_ring_atoms) {
        rmsd_check = check_nt_type_by_rmsd_poly(residue);
        rmsd_result = rmsd_check.rmsd;
        found_purine_atoms = rmsd_check.found_purine_atoms;

        if (!rmsd_result.has_value() || *rmsd_result > RMSD_THRESHOLD) {
            if (found_purine_atoms) {
                auto pyr_rmsd = try_pyrimidine_rmsd_poly(residue);
                if (!pyr_rmsd.has_value() || *pyr_rmsd > RMSD_THRESHOLD) {
                    return result;
                }
                if (!core::ModifiedNucleotideRegistry::contains(res_name)) {
    ...
    // Determine residue type if still unknown
    bool is_registry_nucleotide = core::ModifiedNucleotideRegistry::contains(res_name);

    if (!is_registry_nucleotide && residue_type == core::ResidueType::UNKNOWN) {
        if (has_ring_atoms) {
            if (has_purine_atoms || is_purine_letter(one_letter)) {
                residue_type = determine_purine_type_poly(residue);
            } else {
                residue_type = determine_pyrimidine_type_poly(residue, one_letter);
            }
        } else {
            return result;
        }
    ...
    // For ring atom matching, we need to use the old Residue type temporarily
    // Create a temporary Residue for matching (this is a bridge until RingAtomMatcher is updated)
    core::Residue temp_residue(res_name, residue.seq_num(), residue.chain_id(), residue.insertion());
    for (const auto& atom : residue.atoms()) {
        temp_residue.add_atom(atom);
    }
    ...
    MatchedAtoms matched = RingAtomMatcher::match(temp_residue, standard_template,
                                                  std::optional<core::ResidueType>(matching_type));
