...
void JsonWriter::record_ls_fitting(size_t residue_idx, size_t num_points, double rms_fit,
    ...
    record["translation"] = nlohmann::json::array({
        format_double(translation.x()),
        format_double(translation.y()),
...
void JsonWriter::record_frame_calc(size_t residue_idx, char base_type,
    ...
    for (size_t i = 0; i < matched_std_xyz.size(); ++i) {
        ...
        coord_json["std_xyz"] = nlohmann::json::array({
            format_double(std_xyz.x()),
            format_double(std_xyz.y()),
            format_double(std_xyz.z())
        });
        
        // Experimental coordinates
        const auto& exp_xyz = matched_exp_xyz[i];
        coord_json["exp_xyz"] = nlohmann::json::array({
            format_double(exp_xyz.x()),
            format_double(exp_xyz.y()),
            format_double(exp_xyz.z())
        });
...
void JsonWriter::record_pair_validation(size_t base_i, size_t base_j, bool is_valid,
