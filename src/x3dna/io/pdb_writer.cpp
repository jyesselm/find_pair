...
void PdbWriter::write_file(const core::structure::Structure& structure, const std::filesystem::path& path) {
    std::ofstream file(path);
    if (!file.is_open()) {
        throw std::runtime_error("Cannot open file for writing: " + path.string());
    }
...
void PdbWriter::write_stream(const core::structure::Structure& structure, std::ostream& stream) {
    int atom_serial = 1; // 1-based serial numbers
...
std::string PdbWriter::to_string(const core::structure::Structure& structure) {
    std::ostringstream stream;
    write_stream(structure, stream);
    return stream.str();
}

std::string PdbWriter::format_atom_line(const core::Atom& atom, const core::structure::IResidue& residue,
