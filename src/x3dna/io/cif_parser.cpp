...
core::structure::Structure CifParser::parse_file_poly(const std::filesystem::path& path) {
    if (!std::filesystem::exists(path)) {
        throw ParseError("CIF file does not exist: " + path.string());
    }
...
core::structure::Structure CifParser::parse_string_poly(const std::string& content) {
    try {
        if (content.empty()) {
            throw ParseError("Empty CIF content");
        }
...
core::structure::Structure CifParser::build_poly_structure_from_residues(
