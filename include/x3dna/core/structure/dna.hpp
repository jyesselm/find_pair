/**
 * @file dna.hpp
 * @brief DNA nucleotide residue class
 */
...
/**
 * @class DNA
 * @brief Represents a DNA nucleotide (A, G, C, T or modified)
 */
class DNA final : public INucleotide {
public:
    DNA() = default;

    DNA(const std::string& name, int seq_num, const std::string& chain_id,
    ...
    [[nodiscard]] bool is_dna() const override { return true; }
    ...
    [[nodiscard]] std::unique_ptr<IResidue> clone() const override {
        return std::unique_ptr<IResidue>(new DNA(*this));
    }
