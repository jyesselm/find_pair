...
TEST_F(StructureLegacyOrderTest, ResidueCountMatchesLegacy) {
    ...
    Structure structure = parser.parse_file(pdb_file_);
        ...
        EXPECT_EQ(residues.size(), 1070) 
            << "Residue count should match legacy (1070 for 3G8T)";
}
...
TEST_F(StructureLegacyOrderTest, GetLegacyIdxForResidue) {
    ...
    EXPECT_EQ(legacy_idx, 946) 
        << "get_legacy_idx_for_residue should return 946 for residue 946";
}
