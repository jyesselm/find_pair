...
void print_atom_diff(const nlohmann::json& gen_atom, const nlohmann::json& leg_atom, const std::string& key_str) {
    ...
    auto get_str = [](const nlohmann::json& j, const std::string& key) {
        if (j.is_null() || j.empty()) return std::string("");
    ...
    int gen_seq = (gen_atom.is_null() || gen_atom.empty()) ? 0 : 
                  (gen_atom.contains("residue_seq") && gen_atom["residue_seq"].is_number() ? 
                   gen_atom["residue_seq"].get<int>() : 0);
    int leg_seq = (leg_atom.is_null() || leg_atom.empty()) ? 0 : 
                  (leg_atom.contains("residue_seq") && leg_atom["residue_seq"].is_number() ? 
                   leg_atom["residue_seq"].get<int>() : 0);
    if (gen_seq != leg_seq) {
...
int main(int argc, char* argv[]) {
    ...
    auto gen_atoms = std::find_if(gen_json["calculations"].begin(), gen_json["calculations"].end(),
                                   [](const nlohmann::json& j) { return j.value("type", "") == "pdb_atoms"; });
    ...
    auto get_int = [](const nlohmann::json& j, const std::string& key, int default_val = 0) {
        ...
        if (j[key].is_string()) {
            try { return std::stoi(j[key].get<std::string>()); } catch (...) { return default_val; }
        }
        return default_val;
    };
    ...
    for (const auto& pair : gen_map) gen_keys.insert(pair.first);
    for (const auto& pair : leg_map) leg_keys.insert(pair.first);
    
    auto missing = leg_keys;
    for (const auto& k : gen_keys) missing.erase(k);
    
    auto extra = gen_keys;
    for (const auto& k : leg_keys) extra.erase(k);
