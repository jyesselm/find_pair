    ...
    static std::optional<const Residue*> find_residue_by_legacy_idx(
        ...
        for (const auto& chain : structure.chains()) {
            ...
            for (const auto& residue : chain.residues()) {
                // Match by chain, seq_num, and insertion code (legacy uses ResName + ChainID +
                // ResSeq + iCode)
...
TEST_F(FrameCalculationLegacyTest, CompareSinglePDB) {
    ...
    for (const auto& ls_record : ls_records) {
        ...
        if (ls_record.contains("translation")) {
            std::string error_msg;
            bool translations_match = compare_vectors(result.translation, ls_record["translation"],
                                                      0.05, // Tolerance: 0.05 (relaxed from 0.01)
                                                      error_msg);
...
TEST_F(FrameCalculationLegacyTest, CompareMultiplePDBs) {
    ...
    for (size_t i = 0; i < num_to_test; ++i) {
        ...
        for (const auto& ls_record : ls_records) {
            ...
            if (ls_record.contains("rotation_matrix")) {
                std::string error_msg;
                if (!compare_matrices(result.rotation_matrix, ls_record["rotation_matrix"], 0.05,
                                      error_msg)) { // Relaxed from 0.01
                    all_match = false;
                }
            }
            ...
            if (ls_record.contains("translation")) {
                std::string error_msg;
                if (!compare_vectors(result.translation, ls_record["translation"], 0.05,
                                     error_msg)) { // Relaxed from 0.01
                    all_match = false;
                }
            }
            ...
            if (ls_record.contains("rms_fit") && !ls_record["rms_fit"].is_null()) {
                double legacy_rms = ls_record["rms_fit"].get<double>();
                std::string error_msg;
                if (!compare_doubles(result.rms_fit, legacy_rms, 0.005,
                                     error_msg)) { // Relaxed from 0.001
                    all_match = false;
                }
            }
