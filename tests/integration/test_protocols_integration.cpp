...
protected:
    void SetUp() override {
        ...
        if (pairs_.empty()) {
            GTEST_SKIP() << "No PDB/legacy JSON pairs found for testing. "
                         << "Place legacy JSON files (with _globals.json suffix) in "
                            "data/json_legacy/ to enable tests.";
        }
     ...
     *
     * Legacy find_bestpair_selection files are in
     * data/json_legacy/find_bestpair_selection/PDB_ID.json They are arrays with one record
     * containing a "pairs" array
     */
    std::vector<nlohmann::json> load_legacy_base_pairs(const fs::path& json_file) {
        ...
        try {
            ...
            // Legacy find_bestpair_selection files are in
            // data/json_legacy/find_bestpair_selection/PDB_ID.json These contain the ACTUAL
    ...
    void compare_base_pairs(const std::vector<BasePair>& modern_pairs,
        ...
        for (const auto& [normalized_pair, legacy_json] : legacy_map) {
            ...
            // Determine order: legacy JSON has base_i and base_j, need to match with modern pair
            // order
